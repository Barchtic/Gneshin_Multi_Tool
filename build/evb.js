...
exports.default = async function (context) {
    ...
    await fsex.rename(path.join(distDir, 'Genshin Multi Tool.exe'), path.join(distDir, 'Genshin Multi Tool.exe'))
    ...
    generateEvb(evbFile, path.join(distDir, 'Genshin Multi Tool.exe'), ouptFile, path.join(baseDir, 'win-unpacked'), {
        filter(fullPath, name) {
            if (name === 'Genshin Multi Tool.exe') return false
            return true
        },
        evbOptions: {
            compressFiles: false,
