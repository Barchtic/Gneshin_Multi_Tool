    ...
    methods: {
        ...
        async detectOnce(revertStatus = true, firstRun = true) {
            ...
            if (blocks.length <= 0) {
                ...
                ElMessageBox({
                    type: 'error',
                    title: __('Detection Failure'),
                    message: __('Please make sure you let the switcher fittingly surrounds the list of artifacts area. If you are always prompted for an error, submit feedback.'),
                })
                throw new Error()
            }
            bus.blocks = blocks
            ...
            if (firstRun) {
                ...
                if (bus.rows * bus.cols !== blocks.length) {
                    ...
                    ElMessageBox({
                        ...
                        title: __('Detection Failure'),
                        message: __(
                            'Cant confirm number of rows and columns. Please check if the top of artifacts list aligns to the top of a row of artifacts and current page is not the last page, or go to a dark place and reopen your backpack.',
                        ),
                    })
                    bus.auto = false
                    bus.status = STATUS.ERROR
