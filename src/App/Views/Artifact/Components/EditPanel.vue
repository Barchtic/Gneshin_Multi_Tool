...
<template>
    <el-dialog :title="title" width="400px" :model-value="show" @update:model-value="$emit('update:show', $event)">
        <article class="artiface-edit-panel">
            ...
            <div class="main">
                <el-select v-model="artifact.main.name" size="small" :placeholder="__('Main Stat')">
                    <el-option v-for="(j, a) in ArtifactParamTypes" :key="a" :value="j" :label="__(j)"></el-option>
                </el-select>
                <el-input v-model="artifact.main.value" size="small" :placeholder="__('Attribute Value')"> </el-input>
            </div>
            <br />
            {{ __('Sub Stat') }}<br />
            <el-empty v-if="artifact.sub.length <= 0" :image-size="80" :description="__('No SubStat')"></el-empty>
            <ul v-else class="sub">
                <li v-for="(i, a) in artifact.sub" :key="a">
                    <el-input v-model="i.value" size="small" :placeholder="__('Attribute Value')">
                        <template #prepend>
                            <el-select v-model="i.name" size="small" :placeholder="__('Attribute Name')">
                                <el-option
        ...
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button size="small" type="danger" plain @click="doReset">{{ __('Reset') }}</el-button>
                <el-button size="small" type="primary" @click="doSave">{{ __('Save') }}</el-button>
            </span>
        </template>
    </el-dialog>
</template>
